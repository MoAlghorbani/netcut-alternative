...
def get_gateway_ip():
    ...
    try:
        ...
        if gateway and gateway != '0.0.0.0':
            print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Found gateway IP: {Fore.LIGHTCYAN_EX}{gateway}{Style.RESET_ALL}")
            return gateway
        else:
            raise ValueError("Could not determine gateway from Scapy routes.")
    except Exception as e:
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error getting gateway IP: {e}")
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[i]{Style.RESET_ALL} Attempting fallback method...")
        ...
        try:
                ...
                if 'Default Gateway' in line:
                    ...
                    if len(parts) > 1:
                        ...
                        if gw and gw != '0.0.0.0': # Check if a valid IP is found
                             print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Found gateway IP (fallback): {Fore.LIGHTCYAN_EX}{gw}{Style.RESET_ALL}")
                             return gw
            ...
            print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Fallback failed: {fallback_e}")
            print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Could not determine gateway IP. ARP spoofing will not work.")
            return None
...
def get_mac(ip_address, interface):
    ...
    try:
        ...
        if ans:
            ...
            print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[w]{Style.RESET_ALL} Could not resolve MAC address for {ip_address}. Maybe offline?")
            return None
    except Exception as e:
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error getting MAC for {ip_address}: {e}")
        return None
...
def restore(destination_ip, destination_mac, source_ip, source_mac, interface):
    ...
    try:
        ...
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error sending restore packet to {destination_ip}: {e}")
...
def arp_spoof_thread(target_ip, target_mac, gateway_ip, gateway_mac, interface):
    ...
    print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!] Starting ARP spoof against {target_ip}...{Style.RESET_ALL}")
    try:
        ...
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error in ARP spoofing thread: {e}")
    finally:
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Stopping ARP spoof against {target_ip}. Restoring ARP tables...{Style.RESET_ALL}")
        ...
        if my_mac:
             ...
             print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Could not get own MAC for restore. Manual ARP cache clear might be needed on target/gateway.")
        print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} ARP tables restored for {target_ip}.{Style.RESET_ALL}")
...
def start_spoofing(target_ip, target_mac, gateway_ip, gateway_mac, interface):
    ...
    if spoofing_thread and spoofing_thread.is_alive():
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[w]{Style.RESET_ALL} Spoofing is already running against {spoofing_target['ip']}. Stop it first.")
        return
...
def stop_spoofing():
    ...
    global spoofing_thread, spoof_stop_event, spoofing_target
    if spoofing_thread and spoofing_thread.is_alive():
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Sending stop signal to spoofing thread...{Style.RESET_ALL}")
        ...
        if spoofing_thread.is_alive():
             print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Spoofing thread did not stop gracefully. Restoration might be incomplete.")
        spoofing_thread = None
        spoofing_target = None
    else:
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[i]{Style.RESET_ALL} Spoofing is not currently active.")
...
def start_sniffing(interface):
    ...
    print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Starting packet sniffer (Bandwidth & DNS)...{Style.RESET_ALL}")
    try:
        ...
        sniff(iface=interface, prn=packet_callback, store=0, 
         ...
         print(f"\n{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error starting sniffer on interface {interface}: {e}")
         print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Packet sniffing failed to start.")
        ...
        print(f"\n{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} An unexpected error occurred in the sniffer thread: {e}")
        ...
        if not stop_sniffing_event.is_set(): # Only print if stopped normally
             print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Packet sniffer stopped.{Style.RESET_ALL}")
...
def get_local_ip_range():
    ...
    try:
        ...
        if len(ip_parts) == 4:
            ...
            print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Determined local IP range: {Fore.LIGHTCYAN_EX}{ip_range}{Style.RESET_ALL}")
            return ip_range
        else:
        ...
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Could not automatically determine local IP range: {e}")
        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[!]{Style.RESET_ALL} Please specify the target IP range manually (e.g., 192.168.1.0/24).")
        return None

def list_interfaces():
    """Lists available network interfaces."""
    print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Available network interfaces:")
    ...
    for i, iface_name in enumerate(interfaces):
        try:
            ...
            if iface_details and hasattr(iface_details, 'ip') and iface_details.ip and iface_details.ip != '0.0.0.0':
                print(f"  {Fore.LIGHTYELLOW_EX}{i}{Style.RESET_ALL}: {Fore.LIGHTCYAN_EX}{iface_name}{Style.RESET_ALL} (IP: {iface_details.ip}, MAC: {iface_details.mac})")
             ...
             print(f"  {Fore.LIGHTYELLOW_EX}{i}{Style.RESET_ALL}: {Fore.LIGHTCYAN_EX}{iface_name}{Style.RESET_ALL} ({Fore.LIGHTRED_EX}Error retrieving details: {e}{Style.RESET_ALL})")
    # Return only the interfaces that were printed with details
    # Re-index the valid interfaces for user selection
    print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Selectable interfaces:")
    selectable_map = {}
    for idx, name in enumerate(valid_interfaces):
        print(f"  {Fore.LIGHTYELLOW_EX}{idx}{Style.RESET_ALL}: {Fore.LIGHTCYAN_EX}{name}{Style.RESET_ALL}")
        selectable_map[idx] = name

    if not selectable_map:
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} No suitable network interfaces found for scanning.")
...
def select_interface(interfaces, selectable_map):
    ...
    while True:
        try:
            choice = input(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Enter the number of the interface to use for scanning: ")
            index = int(choice)
            if index in selectable_map:
                selected_iface = selectable_map[index]
                print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Using interface: {Fore.LIGHTCYAN_EX}{selected_iface}{Style.RESET_ALL}")
                ...
                print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Invalid choice. Please enter a number from the selectable list.")
        except ValueError:
            print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Invalid input. Please enter a number.")
        except KeyboardInterrupt:
            print(f"\n{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[!]{Style.RESET_ALL} Scan aborted by user.")
            sys.exit(0)

def arp_scan(ip_range, interface):
    """Performs an ARP scan and updates the known IPs set."""
    global known_ips
    print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Scanning network: {Fore.LIGHTCYAN_EX}{ip_range}{Style.RESET_ALL}...")
    clients = []
    ...
    try:
        ...
        if not answered_list:
             print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[i]{Style.RESET_ALL} No devices responded to ARP scan.")
        ...
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Permission denied. Please run this script with administrator privileges.")
        sys.exit(1)
    except OSError as e:
         # Handle specific OS errors like "Network is down" if srp fails
         print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Network error during scan on interface {interface}: {e}")
         return [] # Return empty list on network error
    except Exception as e:
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} An error occurred during the ARP scan: {e}")
...
def print_results(clients_list):
    ...
    print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Active hosts found:")
    # Added Status column
    ...
    with bandwidth_lock:
        ...
        for client in sorted_clients:
            ...
            if spoofing_target and ip == spoofing_target['ip']:
                status = f"{Fore.LIGHTRED_EX}CUT OFF{Style.RESET_ALL}"
            elif ip == gateway_ip:
            ...
            print(f"{Fore.LIGHTCYAN_EX}{ip:<15}{Style.RESET_ALL}\t{Fore.MAGENTA}{mac:<17}{Style.RESET_ALL}\t{Fore.LIGHTYELLOW_EX}{sent:>10}{Style.RESET_ALL}\t{Fore.LIGHTGREEN_EX}{received:>10}{Style.RESET_ALL}\t{Fore.LIGHTBLUE_EX}{queries_str:<30}{Style.RESET_ALL}\t{status}")

    print(f"{Style.BRIGHT}{Fore.WHITE}{'-' * (len(header) + 15)}{Style.RESET_ALL}") # Adjust separator length

if __name__ == "__main__":
    # --- Add Warning ---
    print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}--- WARNING ---{Style.RESET_ALL}")
    ...
    print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}---------------{Style.RESET_ALL}\n")
    ...
    if not target_range:
        ...
        while not target_range: # Loop until valid input or Ctrl+C
             try:
                 manual_range = input(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Enter the target IP range (e.g., 192.168.1.0/24): ").strip()
                 # Basic validation (optional but recommended)
                 if '/' in manual_range and '.' in manual_range:
                     target_range = manual_range
                 elif not manual_range: # Allow empty input to exit
                      print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} No target range specified. Exiting.")
                      sys.exit(1)
                 else:
                     print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Invalid format. Please use CIDR notation (e.g., 192.168.1.0/24).")
             except KeyboardInterrupt:
                  print(f"\n{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[!]{Style.RESET_ALL} Operation cancelled by user.")
                  sys.exit(0)


    print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Initial scan...")
        ...
        if local_ip_on_iface and local_ip_on_iface != '0.0.0.0':
            with bandwidth_lock:
                if local_ip_on_iface not in known_ips:
                    print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[i]{Style.RESET_ALL} Adding local IP {Fore.LIGHTCYAN_EX}{local_ip_on_iface}{Style.RESET_ALL} to monitored set.")
            ...
            print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[w]{Style.RESET_ALL} Could not get a valid IP for the selected interface ({selected_interface_name}). Local traffic might not be fully tracked.")
    except Exception as e:
        print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Error getting local IP for interface {selected_interface_name}: {e}")
    # ----------------------------------
    ...
    gateway_mac = None
    if gateway_ip:
        print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Attempting to resolve gateway MAC ({gateway_ip})...")
        gateway_mac = get_mac(gateway_ip, selected_interface_name)
        if gateway_mac:
            print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Gateway MAC resolved: {Fore.LIGHTCYAN_EX}{gateway_mac}{Style.RESET_ALL}")
        else:
            print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Failed to resolve gateway MAC. ARP spoofing will likely fail.")
    # ------------------------
    ...
    if stop_sniffing_event.is_set(): # Check if sniffer failed immediately
         print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Bandwidth monitor could not be started. Exiting.")
         sys.exit(1)


    print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Scanner running. Bandwidth monitoring active.")
    print(f"Press '{Fore.LIGHTYELLOW_EX}r{Style.RESET_ALL}' to refresh, '{Fore.LIGHTYELLOW_EX}c{Style.RESET_ALL}' to cut/uncut internet, '{Fore.LIGHTYELLOW_EX}q{Style.RESET_ALL}' to quit.")
    print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[WARNING]{Style.RESET_ALL} ARP Spoofing (cutting internet) should only be used on networks you own or have explicit permission to test.")
    ...
    try:
        while True:
            ...
            if sniffer_alive and not sniffer_thread.is_alive():
                print(f"\n{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Packet sniffer thread stopped unexpectedly.")
                sniffer_alive = False
                # Decide whether to try restarting or just inform the user
                print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[i]{Style.RESET_ALL} Bandwidth and DNS data will not be updated.")
            ...
            if key_pressed == 'r':
                print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Refreshing host list and usage...{Style.RESET_ALL}")
                os.system('cls' if os.name == 'nt' else 'clear') # Clear screen
                discovered_hosts = arp_scan(target_range, selected_interface_name)
                print_results(discovered_hosts) # Print updated list
                print(f"\nPress '{Fore.LIGHTYELLOW_EX}r{Style.RESET_ALL}' to refresh, '{Fore.LIGHTYELLOW_EX}c{Style.RESET_ALL}' to cut/uncut internet, '{Fore.LIGHTYELLOW_EX}q{Style.RESET_ALL}' to quit.")
                if spoofing_target:
                     print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[!] Currently cutting internet for: {Fore.LIGHTCYAN_EX}{spoofing_target['ip']}{Style.RESET_ALL}")
                ...
                if spoofing_target:
                    print(f"\n{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Internet is currently cut for: {Fore.LIGHTCYAN_EX}{spoofing_target['ip']}{Style.RESET_ALL}")
                    ...
                    print(f"\n{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Select a device to cut internet access:")
                    ...
                    for i, client in enumerate(discovered_hosts):
                        ...
                        if client['ip'] != gateway_ip and client['ip'] != local_ip_on_iface:
                            print(f"  {Fore.LIGHTYELLOW_EX}{i}{Style.RESET_ALL}: {Fore.LIGHTCYAN_EX}{client['ip']:<15}{Style.RESET_ALL} ({Fore.MAGENTA}{client['mac']}{Style.RESET_ALL})")
                            host_map[i] = client
                        else:
                    ...
                    if not host_map:
                        print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[w]{Style.RESET_ALL} No other targetable devices found in the current list.")
                    else:
                        while True:
                            try:
                                ...
                                if target_index in host_map:
                                    ...
                                    if not gateway_ip or not gateway_mac:
                                         print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Gateway IP or MAC not found. Cannot initiate spoofing.")
                                         break # Exit selection loop

                                    print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Preparing to cut internet for {Fore.LIGHTCYAN_EX}{target_ip}{Style.RESET_ALL} ({target_mac}).")
                                    ...
                                    if confirm == 'y':
                                        ...
                                        if refreshed_target_mac:
                                            ...
                                            print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Failed to re-confirm MAC for {target_ip}. Aborting.")
                                    else:
                                        print("Operation cancelled.")
                                    break # Exit selection loop after action or cancellation
                                else:
                                    print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Invalid choice.")
                            except ValueError:
                                print(f"{Style.BRIGHT}{Fore.LIGHTRED_EX}[!]{Style.RESET_ALL} Invalid input. Please enter a number or 'b'.")
                            except KeyboardInterrupt:
                                print("\nOperation cancelled.")
                                break
                ...
                print(f"\n{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Quitting...{Style.RESET_ALL}")
        ...
        print(f"\n{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[!]{Style.RESET_ALL} Scan aborted by user.{Style.RESET_ALL}")
    finally:
        # Stop the sniffer thread gracefully
        if sniffer_alive and sniffer_thread.is_alive():
            print(f"{Style.BRIGHT}{Fore.LIGHTYELLOW_EX}[*]{Style.RESET_ALL} Stopping packet sniffer...{Style.RESET_ALL}")
            stop_sniffing_event.set()
        ...
        print(f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}[*]{Style.RESET_ALL} Exiting program.")
